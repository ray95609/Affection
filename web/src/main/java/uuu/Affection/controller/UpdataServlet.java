	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(errList.isEmpty()) {
			...
			try {
				...
				//3.1-內部轉交給Updata_ok.jsp	
				//改外部回到上一頁或回到首頁
				RequestDispatcher dispatcher=request.getRequestDispatcher("/memberUpdate_ok.jsp") ;
                ...
                this.log("會員資料修改發生非預期錯誤"+e.getMessage(), e);
                errList.add("會員資料修改失敗:"+e.getMessage());
            }
        }
		request.setAttribute("errList_Update", errList);
        //3.2 顯示註冊失敗畫面
		RequestDispatcher dispatcher=request.getRequestDispatcher("/memberCenter.jsp") ;
		dispatcher.forward(request, response);
